...
export default function RunPage() {
  ...
  async function startRun() {
    await fetch(`/api/runs/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "IN_PROGRESS" }) });
    load();
  }
    ...
    <div className="p-8">
      ...
      <div className="space-y-3 max-w-3xl">
        {run.steps.map((step: any, index: number) => (
          <div key={step.id} className={`card ${step.status === "PASSED" ? "border-green-200" : step.status === "FAILED" ? "border-red-200" : step.status === "BLOCKED" ? "border-orange-200" : ""}`}>
            <div className="p-4">
              <div className="flex items-start gap-4">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${
                  ...
                  {step.status === "PASSED" ? "✓" : step.status === "FAILED" ? "✗" : step.status === "BLOCKED" ? "!" : index + 1}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {step.doneAt && (
                    <div className="mt-1 text-xs text-slate-400">Uitgevoerd: {formatDateTime(step.doneAt)}</div>
                  )}
                </div>
              </div>

              {/* Execution controls */}
              {run.status === "IN_PROGRESS" && activeStep !== step.id && (
                <div className="flex items-center gap-2 mt-3 ml-12">
                  <button onClick={() => { setActiveStep(step.id); setStepForm({ status: "PASSED", result: "", notes: "" }); }} className="text-xs btn-secondary">
                    Resultaat vastleggen
                  </button>
                  <button onClick={() => setShowIssueFor(step.id)} className="text-xs text-red-600 hover:text-red-700 border border-red-200 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors">
                    Bevinding melden
                  </button>
                </div>
              )}
              ...
              {activeStep === step.id && (
                <div className="mt-3 ml-12 space-y-3 p-3 bg-slate-50 rounded-lg">
                  ...
                  <input className="input text-sm" placeholder="Resultaat (optioneel)" value={stepForm.result} onChange={e => setStepForm({...stepForm, result: e.target.value})} />
                  <textarea className="input text-sm resize-none" rows={2} placeholder="Notities (optioneel)" value={stepForm.notes} onChange={e => setStepForm({...stepForm, notes: e.target.value})} />
                  <div className="flex gap-2">
                    <button onClick={() => updateStep(step.id)} disabled={saving || !stepForm.status} className="btn-primary text-xs">{saving ? "Opslaan..." : "Opslaan"}</button>
                    <button onClick={() => setActiveStep(null)} className="btn-secondary text-xs">Annuleren</button>
                  </div>
                </div>
              )}
              ...
              {showIssueFor === step.id && (
                <div className="mt-3 ml-12 space-y-3 p-3 bg-red-50 rounded-lg border border-red-200">
                  ...
                  <input className="input text-sm" placeholder="Titel *" value={issueForm.title} onChange={e => setIssueForm({...issueForm, title: e.target.value})} />
                  <textarea className="input text-sm resize-none" rows={3} placeholder="Beschrijving *" value={issueForm.description} onChange={e => setIssueForm({...issueForm, description: e.target.value})} />
                  <div className="flex gap-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => createIssue(step.id)} disabled={saving || !issueForm.title || !issueForm.description} className="btn-danger text-xs">{saving ? "Melden..." : "Bevinding melden"}</button>
                    <button onClick={() => setShowIssueFor(null)} className="btn-secondary text-xs">Annuleren</button>
                  </div>
                </div>
              )}
              ...
              {step.issues?.length > 0 && (
                <div className="mt-3 ml-12 space-y-2">
                  {step.issues.map((issue: any) => (
                    <Link key={issue.id} href={`/issues/${issue.id}`} className="flex items-center gap-3 p-2 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
